...
def init_db():
    ...
    connection.executescript(
        ...
        CREATE TABLE IF NOT EXISTS mensagens (
            ...
            atualizada_em TEXT NOT NULL, lida_em TEXT DEFAULT ''
        );
        """ % (
...
def dashboard():
    ...
    message_rows = connection.execute(
        ...
        "WHERE m.remetente_id = ? OR m.destinatario_id = ? ORDER BY m.id DESC LIMIT 30",
...
def criar_mensagem():
    ...
    cursor = connection.execute(
        "INSERT INTO mensagens(remetente_id, destinatario_id, assunto, mensagem, criada_em, atualizada_em) VALUES (?, ?, ?, ?, ?, ?)",
...
def editar_mensagem(message_id):
    ...
    message = connection.execute(
        "SELECT m.*, recipient.nome destinatario FROM mensagens m JOIN users recipient ON recipient.id = m.destinatario_id WHERE m.id = ? AND m.remetente_id = ?",
    ...
    if request.method == "POST":
        ...
        if not body or len(body) > 5000:
            ...
            connection.execute(
                ...
                (subject[:160], body, datetime.now().isoformat(timespec="seconds"), message_id),
            )
            connection.commit()
            connection.close()
...
def grupos():
    ...
    for frequency in group_frequency_rows:
        frequencies_by_participant.setdefault((frequency["grupo_id"], frequency["pessoa_id"]), {})[frequency["encontro"]] = dict(frequency)
    groups = []
    for group in group_rows:
        ...
        for participant in group_data["participants"]:
            participant["frequencies"] = frequencies_by_participant.get((group["id"], participant["pessoa_id"]), {})
        registered = {encounter for participant in group_data["participants"] for encounter, frequency in participant["frequencies"].items() if frequency["status"]}
...
def formar_grupo():
    ...
    people = connection.execute(f"SELECT id FROM pessoas WHERE id IN ({placeholders}) AND grupo_responsabilizacao <> ''", person_ids).fetchall()
    already_grouped = connection.execute(f"SELECT pessoa_id FROM grupo_participantes WHERE pessoa_id IN ({placeholders})", person_ids).fetchall()
    if len(people) != len(person_ids) or already_grouped:
        ...
        return redirect(url_for("grupos"))
    group_name = request.form.get("nome", "").strip() or f"Grupo reflexivo {connection.execute('SELECT COUNT(*) AS total FROM grupos_reflexivos').fetchone()['total'] + 1}"
    group_cursor = connection.execute("INSERT INTO grupos_reflexivos(nome, status, criado_em, criado_por) VALUES (?, ?, ?, ?)", (group_name, "Em andamento", datetime.now().isoformat(timespec="seconds"), session["uid"]))
    group_id = group_cursor.lastrowid
    for person_id in person_ids:
        connection.execute("INSERT INTO grupo_participantes(grupo_id, pessoa_id) VALUES (?, ?)", (group_id, person_id))
    connection.commit()
    connection.close()
...
def salvar_frequencia_grupo(group_id):
    ...
    facilitator_names = [name.strip() for name in request.form.getlist("facilitador") if name.strip()]
    ...
    participants = connection.execute("SELECT pessoa_id FROM grupo_participantes WHERE grupo_id = ?", (group_id,)).fetchall()
    if not group:
        connection.close()
        return "Grupo não encontrado", 404
    for participant in participants:
        ...
        connection.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(grupo_id, pessoa_id, encontro) DO UPDATE SET "
            "status = excluded.status, data = excluded.data, horario = excluded.horario, "
