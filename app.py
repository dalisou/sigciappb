...
def init_postgres_db():
    ...
    statements = [
        ...
        """CREATE TABLE IF NOT EXISTS grupo_frequencias (
            ...
            data TEXT DEFAULT '', UNIQUE(grupo_id, pessoa_id, encontro)
        )""",
...
def salvar_frequencia_grupo(group_id):
    ...
    for participant in participants:
        ...
        if status not in FREQUENCY_STATUS_OPTIONS:
            ...
            return redirect(url_for("grupos"))
        connection.execute("INSERT INTO grupo_frequencias(grupo_id, pessoa_id, encontro, status, data) VALUES (?, ?, ?, ?, ?) ON CONFLICT(grupo_id, pessoa_id, encontro) DO UPDATE SET status = excluded.status, data = excluded.data", (group_id, person_id, encounter, status, attendance_date))
    connection.commit()
    connection.close()
